...
## Addendum (2026-07-20) — Drag-hover-fill-before-commit question, re-verified

Re-researched specifically: "fill the icon under the Liquid Glass lens while dragging, before the tab is committed, revert if not committed." Confirmed answer: **NOT feasible** with native `TabView`/`UITabBarController` or `react-native-bottom-tabs`.

- No Apple API/environment value/delegate exposes a live "touch currently over tab N, not yet selected" state. Selection is a discrete, atomic tap-commit event system-wide — confirmed by direct fetch of Donny Wals "Exploring tab bars on iOS 26 with Liquid Glass" (https://www.donnywals.com/exploring-tab-bars-on-ios-26-with-liquid-glass/) and Swift with Majid "Glassifying tabs in SwiftUI" (https://swiftwithmajid.com/2025/06/24/glassifying-tabs-in-swiftui/) — neither mentions any hover/drag-preview mechanic; both only cover `.tabBarMinimizeBehavior`, accessory views, search-tab role, programmatic selection binding.
- Confirmed exact source file paths (previously approximate): `.noneSymbolVariant()` lives in `packages/react-native-bottom-tabs/ios/TabViewImpl.swift`; per-tab label view is `packages/react-native-bottom-tabs/ios/TabItem.swift`. The library's `isFocused` is driven purely by the committed `selectedIndex` from JS — no drag-gesture recognizer in the library reports intermediate touch position, because `UITabBarController`/`TabView` owns touch routing internally and only reports final `didSelect`.
- To get literal fill-on-hover-during-drag, the only path is abandoning the native Liquid Glass bar entirely for a custom `DragGesture`/`PanGestureHandler` bar that hit-tests touch x-position against icon frames per-frame — same pattern as `SwipeableTabBarController` (https://github.com/marcosgriselli/SwipeableTabBarController) and `union-tab-view` (https://github.com/unionst/union-tab-view), both of which replace rather than hook into the native bar. Not recommended for Swellyo since `react-native-bottom-tabs` was adopted specifically for native Liquid Glass appearance — building a custom bar to get this one micro-interaction gives up everything the library was chosen for (ties to [[research_instagram_liquid_glass_rollback]]).
