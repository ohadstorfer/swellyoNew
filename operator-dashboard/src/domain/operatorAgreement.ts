 ...
 *
 * ── Duplicated in the app as `src/services/terms/operatorAgreement.ts` ────────
 * The app is the original; this is a byte-for-byte copy of everything below
 * the header, and `operatorSetup.test.ts` fails the build if the two drift.
 * Two surfaces showing two different summaries, or recording two different
 * versions, is precisely the bug that test exists for: on 6 September 2026 the
 * app had moved to `summary-2026-08-12` while this site still said "the terms
 * are not published yet" and recorded `placeholder-2026-08-11`, so agreeing on
 * either surface left the step unfinished on the other.
 *
 * WHEN THE REAL AGREEMENT ARRIVES, in BOTH files:
 *   1. replace `SECTIONS` with the real text (or a fetch of the hosted doc),
 *   2. set `IS_DRAFT` to false,
 *   3. bump `OPERATOR_TERMS_VERSION` — app `services/trips/operatorSetup.ts`
 *      and this project's `domain/operatorSetup.ts`, together.
