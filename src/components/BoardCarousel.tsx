...
export const BoardCarousel: React.FC<BoardCarouselProps> = ({
    ...
    return (
      <View style={[styles.carouselItem, { width: itemWidth }]}>
        <Image
          ...
          resizeMode="contain"
          onError={(e) => {
            console.error('Failed to load board image:', item.imageUrl);
          }}
        />
      </View>
    );
  ...
  React.useEffect(() => {
    if (flatListRef.current && activeIndex >= 0) {
      // Use longer timeout for web to ensure layout is complete
  ...
  React.useEffect(() => {
    ...
    if (newIndex >= 0 && newIndex !== activeIndex && flatListRef.current) {
      ...
      setTimeout(() => {
        flatListRef.current?.scrollToIndex({
          index: newIndex,
          animated: Platform.OS !== 'web', // Disable animation on web for better UX
          viewPosition: 0.5,
        });
      }, timeout);
    }
  ...
  // Navigation functions for web arrows
    ...
    <View style={styles.container}>
      <View style={styles.carouselWrapper}>
        ...
        <FlatList
          ...
          horizontal
          pagingEnabled={Platform.OS !== 'web'} // Disable paging on web, use snap instead
          ...
          onScrollToIndexFailed={(info) => {
            // Fallback for web when scrollToIndex fails
          ...
          // Web-specific style for better scrolling
          {...(Platform.OS === 'web' && {
...
const styles = StyleSheet.create({
  ...
  arrowButton: {
    ...
    top: '50%',
    marginTop: -20, // Half of button height (40/2) to center vertically
    zIndex: 10,
  ...
  carouselContent: {
    ...
    ...(Platform.OS === 'web' && {
      // Center the carousel items on web
      paddingLeft: 0,
      paddingRight: 0,
    }),
  },
  ...
  boardImage: {
    ...
    ...(Platform.OS === 'web' && {
      // Ensure images don't get cut off on web
