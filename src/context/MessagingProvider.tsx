        ...
        if (conv?.last_message?.id === messageId) {
          try {
            ...
            const { data: previousMessages, error } = await supabase
              .from('messages')
              .select('id, conversation_id, sender_id, body, attachments, is_system, edited, deleted, created_at, updated_at, type, image_metadata, video_metadata, file_metadata, contact_metadata')
              .eq('conversation_id', conversationId)
              .eq('deleted', false)
