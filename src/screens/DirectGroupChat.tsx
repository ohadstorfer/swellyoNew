...
export const DirectGroupChat: React.FC<DirectGroupChatProps> = ({
  ...
  // fetched fresh from the DB at open — racing (and in practice beating, 2s
  // trailing debounce) the markAsRead write that will clobber it. If the fetch
  // loses that race the watermark reads as "now", zero messages qualify, and
  // the divider is simply absent — never wrong.
  ...
  const renderItem = useCallback(({ item, index }: { item: ChatDisplayRow; index: number }) => {
    ...
    const olderSenderId = rowSender(displayRows[index + 1]);
    const isFirstInRun = olderSenderId === null || olderSenderId !== rowSenderId;
