...
export const DirectMessageScreen: React.FC<DirectMessageScreenProps> = ({
        ...
        const launchNativeImagePicker = async () => {
          try {
            ...
            if (!result.canceled && (result.assets?.length ?? 0) > 1) {
              ...
              if (__DEV__) console.log('[DirectMessageScreen] multi-pick →', items.length, 'items');
            ...
            if (uri && !canceled) {
              ...
              if (isVideo) {
                ...
                console.log('[DirectMessageScreen] video picker returned', {
                  platform: Platform.OS,
                  width: asset?.width ?? 0,
  ...
  // ─── File attachments ─────────────────────────────────────────────────────
  // Upload-first file helper (mirrors uploadAndCreateImage). Uploads the bytes,
  // then writes the row only on success — a failed send leaves nothing on the
  // server. Returns the created row + the finalized file_metadata.
  ...
  const handleVideoSend = async (caption?: string, overrideVideoUri?: string) => {
    ...
    // Tap-to-bubble. Should read ~0ms on the hinted path; anything else means
    // something started awaiting ahead of the bubble again.
    console.log(`[videoSend] bubble on screen in ${Date.now() - sendAt}ms`);
    scrollToBottom();
    ...
    return (
      ...
      >
        <SafeMessageBubble messageId={message.id}>
          {renderMessage(message, isLastInRun)}
        </SafeMessageBubble>
      </Reanimated.View>
    );
...
const styles = StyleSheet.create({
  ...
  messageImage: {
    width: '100%',
    // No minHeight: width:'100%' already guarantees the image fills the bubble
    // width, so a floor here would only override the dynamic aspectRatio and
    // force wide/panorama images into a too-tall box (then cover-crop zooms them).
    // Let the clamped aspectRatio govern the height; maxHeight caps tall images.
    maxHeight: 500,
  ...
  uploadOverlay: {
    ...
    bottom: 0,
    backgroundColor: 'rgba(0, 0, 0, 0.5)',
    // Match messageImage's 13px radius exactly so the overlay's corners sit
    // flush with the photo instead of poking out.
    borderRadius: 13,
    overflow: 'hidden',
    justifyContent: 'center',
    alignItems: 'center',
