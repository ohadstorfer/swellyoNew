...
interface CreateTripFlowBCProps {
  hostId: string | null;
  onCreated: () => void;
  onCancel: () => void;
  /**
  ...
  /**
   * The hosting style chosen in the entry chooser (create mode only). The
   * selector step is hoisted to CreateTripWizard, so this flow starts at the
   * first content step with the style preset. In edit mode it comes from the trip.
   */
  initialHostingStyle?: HostingStyle;
}
...
// The hosting-style selector is hoisted to the entry component (CreateTripWizard),
// so this flow no longer includes it as a step — it starts at 'title'.
const STEPS = [
  'title',
  'heroImage',
  'description',
...
// Main wizard
// ---------------------------------------------------------------------------
export default function CreateTripFlowBC({
  hostId,
  onCreated,
  onCancel,
  initialTrip,
  initialHostingStyle,
}: CreateTripFlowBCProps) {
  const editMode = !!initialTrip;
  const [state, setState] = useState<WizardState>(
    initialTrip
      ? stateFromTrip(initialTrip)
      : { ...INITIAL_STATE, hostingStyle: initialHostingStyle ?? null }
  );
