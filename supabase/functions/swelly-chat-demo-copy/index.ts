...
/**
 * Use a separate LLM call to match lifestyle keywords to image filenames.
 * Returns a map of keyword -> filename or null.
 */
  ...
  try {
    ...
    if (path.endsWith('/new_chat') && req.method === 'POST') {
      ...
      if (userProfile?.pronoun) {
        const pronounInstructions = getPronounInstructions(userProfile.pronoun, userProfile.name)
